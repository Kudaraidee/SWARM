...
if ($Name -in $(arg).PoolName) {
    ...
    try { $Pool_Request = Invoke-RestMethod "https://miningpoolfun.ddns.net/api/currencies" -UseBasicParsing -TimeoutSec 30 -ErrorAction Stop }
    catch {
    ...
    $Fee_Table = $(vars).FeeTable.miningpoolfun;
    $Divisor_Table = $(vars).divisortable.miningpoolfun;
    ...
    $Pool_Data = $Algos | ForEach-Object -Parallel {
        ...
        $To_Add | ForEach-Object { 
            $Pool_Port = $_.port
            $Pool_Host = "miningpoolfun.ddns.net$sub"
