...
if ($Name -in $(arg).PoolName) {
    ...
    try { $Pool_Request = Invoke-RestMethod "https://pool.rwinfo.club/api/currencies" -UseBasicParsing -TimeoutSec 30 -ErrorAction Stop }
    catch {
    ...
    $Fee_Table = $(vars).FeeTable.rwinfo;
    $Divisor_Table = $(vars).divisortable.rwinfo;
