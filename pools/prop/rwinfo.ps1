...
if ($Name -in $(arg).PoolName) {
    ...
    try { $Pool_Request = Invoke-RestMethod "https://pool.rwinfo.club/api/status" -UseBasicParsing -TimeoutSec 30 -ErrorAction Stop } 
    ...
    $Pool_Data = $Pool_Sorted | ForEach-Object -Parallel {
        ...
        $Pool_Port = $_.port
        $Pool_Host = "pool.rwinfo.club$sub"
        ...
        #$D_Table.rwinfo.Add($_.Name, $_.mbtc_mh_factor)
        #$F_Table.rwinfo.Add($_.Name, $_.Fees)
