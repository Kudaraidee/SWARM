...
if ($Name -in $(arg).PoolName) {
    ...
    try { $Pool_Request = Invoke-RestMethod "http://www.phi-phi-pool.com/api/status" -UseBasicParsing -TimeoutSec 10 -ErrorAction Stop } 
    ...
    $Pool_Data = $Pool_Sorted | ForEach-Object -Parallel {
        ...
        $Pool_Port = $_.port
        $Pool_Host = "${reg}.phi-phi-pool.com${sub}"
        ...
        $D_Table.phiphipool.Add($_.Name, $_.mbtc_mh_factor)
        $F_Table.phiphipool.Add($_.Name, $_.Fees)
