...
$(vars).NVIDIATypes | ForEach-Object {
    ...
    if ($(vars).nvidia.'cxaya'.$ConfigType) { $Path = "$($(vars).nvidia.'cxaya'.$ConfigType)" }
    else { $Path = "None" }
    if ($(vars).nvidia.'cxaya'.uri) { $Uri = "$($(vars).nvidia.'cxaya'.uri)" }
    else { $Uri = "None" }
    if ($(vars).nvidia.'cxaya'.minername) { $MinerName = "$($(vars).nvidia.'cxaya'.minername)" }
    else { $MinerName = "None" }

    $User = "User$Num"; $Pass = "Pass$Num"; $Name = "cxaya-$Num"; $Port = "5600$Num";
        ...
        if (
            ...
            $Pools | Where-Object Algorithm -eq $MinerAlgo | ForEach-Object {
                ...
                [PSCustomObject]@{
                    ...
                    Version    = "$($(vars).nvidia.'cxaya'.version)"
                    DeviceCall = "ccminer"
