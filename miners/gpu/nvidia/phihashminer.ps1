...
$(vars).NVIDIATypes | ForEach-Object {
    ...
    if ($(vars).nvidia.phihashminer.$ConfigType) { $Path = "$($(vars).nvidia.phihashminer.$ConfigType)" }
    else { $Path = "None" }
    if ($(vars).nvidia.phihashminer.uri) { $Uri = "$($(vars).nvidia.phihashminer.uri)" }
    else { $Uri = "None" }
    if ($(vars).nvidia.phihashminer.minername) { $MinerName = "$($(vars).nvidia.phihashminer.minername)" }
    else { $MinerName = "None" }

    $User = "User$Num"; $Pass = "Pass$Num"; $Name = "phihashminer-$Num"; $Port = "5900$Num"
    ...
    $MinerConfig = $Global:config.miners.phihashminer
    ...
    $MinerConfig.$ConfigType.commands | Get-Member -MemberType NoteProperty | Select-Object -ExpandProperty Name | ForEach-Object {
        ...
        if (
            ...
            $Pools | Where-Object Algorithm -eq $MinerAlgo | ForEach-Object {
                ...
                if ($_.Worker) { $tcp = 'stratum1+ssl'; $work = $_.Worker }
                else { $tcp = 'stratum2+tcp'; $work = "$($_.$Pass):x" }
                [PSCustomObject]@{
                    MName      = $Name
                    Coin       = $(vars).Coins
                    ...
                    Version    = "$($(vars).nvidia.phihashminer.version)"
