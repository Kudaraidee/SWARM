...
$(vars).CPUTypes | ForEach-Object {
    ...
    if ($(vars).cpu.kudaraidee.$ConfigType) { $Path = "$($(vars).cpu.kudaraidee.$ConfigType)" }
    else { $Path = "None" }
    if ($(vars).cpu.kudaraidee.uri) { $Uri = "$($(vars).cpu.kudaraidee.uri)" }
    else { $Uri = "None" }
    if ($(vars).cpu.kudaraidee.minername) { $MinerName = "$($(vars).cpu.kudaraidee.minername)" }
    else { $MinerName = "None" }
    ...
    $MinerConfig = $Global:config.miners.kudaraidee
        ...
        if ( 
            ...
            $Pools | Where-Object Algorithm -eq $MinerAlgo | ForEach-Object {
                ...
                [PSCustomObject]@{
                    ...
                    Version    = "$($(vars).cpu.kudaraidee.version)"
